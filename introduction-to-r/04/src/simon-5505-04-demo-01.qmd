...
ti_3 |>
	count(sex, survived) -> basic_counts
basic_counts
```
...
basic_counts |>
	pivot_wider(
		names_from="survived",
		values_from="n") -> basic_table
basic_table
```

## Add sex totals and percents
...
basic_counts |>
	group_by(sex) |>
	mutate(total=sum(n)) |>
	mutate(pct=round(100*n/total)) -> sex_totals
sex_totals
```
...
sex_totals |>
	...
	pivot_wider(
		names_from="survived",
		values_from="pct") -> table_1
table_1
```

## Add survived totals and percents

```{r}
#| label: survived-totals

basic_counts |>
	group_by(survived) |>
	mutate(total=sum(n)) |>
	mutate(pct=round(100*n/total)) -> survived_totals
survived_totals
```
