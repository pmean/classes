...
+ Short and fat disadvantages:
  + hard to read 
    +  excessive need to scroll left and right
...
Here is the data in the short and fat format. It's a small dataset so it doesn't look very fat. But we can make it taller and thinner.
...
This is the code to read in the data. With multiple spaces between data values, the read_table function is a good choice.
...
This is the structure of the data. Eight rows, one for each subject. The values are stretched horizontally across three columns, and there is an additional column to indicate which subject is which. This column is not really needed for the short and fat format, but it becomes important when you convert it to a tall and thin format.

You can convert this to a tall and thin format by stacking the running, walking, and cycling values in a single column. You'll need a new variable to remind you whether the value you are converting comes from the first row, second row, etc.
...
The pivot_longer function does the conversion. The first argument is the name of the dataset. The second is the names of the columns that you wish to re-arrange. The names_to argument provides a name to the new column that indicates what type of activity is represented by a particular row. The values_to argument provides a name to the new column that holds the data.
...
This is the structure of the new data. The "activity" variable is the name of the column where the data value came from and "energy" variable is the data value itself. Any variable or variables left out of the list are treated like an identifier for a particular row.

Notice that the tall and thin format has 24 rows, not 8, because each of the 8 subjects now have three rows of data.
...
There is no subject id for each row of the data set. That's okay for the short and fat format. But for the tall and thin format, where there are multiple rows for each subject, you need an id variable.

The contains function creates a variable list of any variable that has the text "Week". That's a lot shorter than typing c(Week0, Week3, Week6, Week9, Week12). The "time" variable identifies which column (Week0, Week3, etc.) that data originally came from and the "depression" variable contains the actual data.
...
Here's the structure of the tall and thin format. 
...
This is the code to produce a set of boxplots. 
...
The case_when function allows you to insert a leading zero for the single digit weeks.

</div>

### Boxplot, revised code (2/2)
...
This is the code for boxplots with the modified names.
...
Here is the code for reading in the backpain data. The second column of data is character and the other columns are numeric.
...
The pivot_wider function converts a tall and thin format into a short and fat format. Notice that there is a "names_from" argument instead of "names_to" and a "values_from" instead of "values_to".
...
Here is the structure of the modified data in short and fat format.
...
age_range <- range(c(
  pain_wide$Age_Pain, 
  pain_wide$Age_NoPain, 
  pain_wide$Age_Sedentary))
```
...
### Backpain plot for height

```{r agreement-4, echo=FALSE}
height_range <- range(c(
  pain_wide$Height_Pain, 
  pain_wide$Height_NoPain, 
  pain_wide$HeightAge_Sedentary))
agreement_height <- ggplot(pain_wide, 
  aes(x=Height_Pain, 
      y=Height_NoPain, 
      label=Match)) +
    geom_text(color="darkgreen") +
    geom_text(aes(y=Height_Sedentary), color="red") + 
    ylab("Height_Sedentary (red) and Height_NoPain (green)" )
agreement_height <-
  agreement_height + 
    expand_limits(
      x=height_range, 
    ...
    geom_segment(
      ...
      yend=age_range[2])
ggsave(
  "../images/agreement_height.png",
  agreement_height, width=4, height=4)
```

![Plot showing agreement in heights](../images/agreement_height.png)

<div class="notes">

This code shows the agreement between the height of the patients in the NoPain group and the patients in the Pain group.
