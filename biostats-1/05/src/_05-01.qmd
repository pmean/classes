...
![](../images/plot01.png "Scatterplot showing a weak relationship between mother's age and duration of breast feeding")
...
![](../images/plot02.png "Same image, with a trend line added")
...
![](../images/plot03.png "Same image with regression trend line extrapolated down to X=0")
...
-   Linear regression splits the observed value into two parts
    -   $Y_i =  \hat{Y}_i + e_i$
    -   $\hat{Y}_i$ is explained portion
    -   $e_i$ is unexplained portion
...
![](../images/plot11.png "Scatterplot of Bush votes versus Buchanan votes")
...
Notice the one point very hight in the middle of the graph. This is Palm Beach County. Buchanan got over 3,000 votes there and this is not at all similar to the Buchanan votes in any other county. 
:::

## Residuals, 4

-   Buchanan_votes = 50 + 5 * Bush_count_thousands (rounding liberally)
    -   The estimated average number of votes for Buchanan increase by 5 for every increase of one thousand votes for Bush.
    
## Residuals, 5
-   (Rounding liberally) In Palm Beach County
    -   Votes (Bush) = 150 thousand
    -   Predicted Votes (Buchanan) = 800
        -   50 + 5*150
    -   Actual Votes (Buchanan) = 3,400
-   Residual = 3,400 - 800 = 2,600

:::notes
We can compute a predicted number of votes for Buchanan for each county by using the above equation. Palm Beach County had 150 thousand votes for Bush. So the regression model would predict that Buchanan should get:

50 + 5 * 150 = 800 votes

Thus, if the relationship observed across the entire state held exactly in Palm Beach County, then we would estimate the vote count for Buchanan to be around 800.

There were actually 3,400 votes recorded for Buchanan, which is quite a discrepancy from what we predicted. The residual, the difference between what we observed and what would be predicted by the regression model is:
...
:::notes
Here are the predicted and residual calculations for the breast feeding study. Looking at just the 20 year old mothers, the predicted value is 13.7. The first mother stopped at 5 weeks, 8.7 weeks short of the predicted value. The second mom was a champ. She stopped only after 29 weeks which is 15.3 weeks above the prediction. The third mother also stopped early. Her residual is -7.7, meaning that her actual stopping time was 7.7 weeks below what we might have predicted. The fourth mother was just a small amount below the prediction, and has a residual of -1.7 weeks. 
:::