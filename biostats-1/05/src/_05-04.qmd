...
-   Deviation from zero in the predicted values
...
## Fundamental property of linear regression

```{=tex}
\begin{matrix}
\hat{Y}_i                     & + & e_i                    & = & Y_i \\
\Sigma(\hat{Y}_i - \bar{Y})^2 & + & \Sigma e_i^2           & = & \Sigma(Y_i - \bar{Y})^2 \\
SS_{Regression}               & + & SS_{Error}             & = & SS_{Total} \\
