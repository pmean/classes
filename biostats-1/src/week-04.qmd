...
-   Accept $H_0$ if t is close to zero.
    -   \$-2 \< t \< 2 or
    -   \$-Z\_{\alpha/2} \< t \< Z\_{\alpha/2} or
    -   \$-t\_{\alpha/2; n-1} \< t \< t\_{\alpha/2; n-1}
...
![xkcd cartoon about jelly beans and cancer](../images/xkcd-jelly-beans.png){#fig-jelly}
...
![Figure 2. Cartoon showing interpretation of various p-values](../images/xkcd-p-values.png){#fig-pvalue}
...
-   $Z\ or\ t = \frac{statistic-parameter}{s.e.(statistic)}$

Specific standardization for the mean

-   $Z\ or\ t = \frac{\bar{X}-\mu}{s.e.(\bar{X})}$
...
-   $P[-t(\alpha/2; n-1) < \frac{\bar{X}-\mu}{s.e.(\bar{X})} < t(\alpha/2; n-1)] = 1-\alpha$

-   $P[-t(\alpha/2; n-1)s.e.(\bar{X}) < \bar{X}-\mu < t(\alpha/2; n-1)s.e.(\bar{X})] = 1-\alpha$

-   $P[\bar{X}-t(\alpha/2; n-1)s.e.(\bar{X}) < \mu < \bar{X} + t(\alpha/2; n-1)s.e.(\bar{X})] = 1-\alpha$

If n > 30 

-   $P[\bar{X}-Z(\alpha/2)s.e.(\bar{X}) < \mu < \bar{X} + Z(\alpha/2)s.e.(\bar{X})] \approx 1-\alpha$
...
-   $\bar{X}-t(\alpha/2; n-1)s.e.(\bar{X})$ and 
-   $\bar{X}+t(\alpha/2; n-1)s.e.(\bar{X})$ 

If n > 30, we have 1-$\alpha$ level of confidence that the population mean lies between

-   $\bar{X}-Z(\alpha/2)s.e.(\bar{X})$ and 
-   $\bar{X}+Z(\alpha/2)s.e.(\bar{X})$
...
![Table 1 from Mondal 2023](../images/chatgpt-01.png){#fig-chat01}
...
-   Results from Table 1 and paper
    -   $\bar{X}=46.94,\ S = 8.23,\ n=14$
-   Calculations
    -   $s.e.(\bar{X})=\frac{8.23}{\sqrt{14}}=2.19956$
    -   $t(0.025, 13) = 2.16$ 
...
-   Results from Table 1 and paper
    -   $\bar{X}=27.07,\ S = 11.46,\ n=14$
-   Calculations
    -   $s.e.(\bar{X})=\frac{11.46}{\sqrt{14}}=3.062814$
    -   $t(0.025, 13) = 2.16$ 
...
-   Cohen's d = $\frac{\bar{X}-C}{s.e.(\bar{X})}$
-   Useful for
...
The first variable is the sample number (1 to 15). The second variable is the weight of an adult female (inches). The third variable is the weight (pounds). The variable names are not included at the top of the data.
...
![SPSS one-sample t-test output](../images/baseball-analysis-03.png){#fig-ball03}
...
-   Note that $s.e.(\bar{X}) \approx 0.007$
    -  9.118 - 0.014 = 9.104
    -  9.118 + 0.014 = 9.132
...
![SPSS import dialog box](../images/baseball-analysis-04.png){#fig-ball04}
...
![SPSS import dialog box](../images/baseball-analysis-05.png){#fig-ball05}
