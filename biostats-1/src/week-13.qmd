...
::: notes
When you are computing more than one correlation, you have to face how to handle missing values right away. The two choices you are offered are pairwise analysis and complete case analysis. Consider a simple scenario with three variables: A, B, and C. Assume that you have all the data (6 rows) for A and B, but only 5 rows for C because the last value is missing. 
:::
...
For complete case analysis, you toss out the row where C is missing entirely. All correlations, correlations between A and B, between A and C, and between B and C are all based on only five observations.
...
You get a bit more precision with pairwise deletion, but it may cause problems if you do further analyses based on the correlations. In particular, factor analysis (a method beyond the scope of this class), can sometimes produce non-sensical results using correlations with pairwise deletion.
:::

## SPSS correlation analysis with pairwise deletion

![](../images/sleep-analysis-03.png)

::: notes
Here is a set of correlations from SPSS using pairwise deletion. 
:::

## SPSS analysis with complete cases
