...
-   table_06: Two factor model ANOVA table
-   plot_03: Line plot for Exercise data
-   plot_04: Alternative line plot for Exercise data
...
ex_1 |>
  ...
  ylab("Average weight loss") -> plot_3
plot_3
```

#### Comment on the code

When you use group_by with two arguments, you have a couple of options. You can continue with a second summary on just the first argument. This would useful if you wanted to compute averages by both groups and then average the averages by the first group.

Alternately, you could just stop and remove the group_by condition before doing any further analysis. This makes sense if you want to manipulate or display the averages as is.

The default in R is the first option, and R provides a warning message that the data remains grouped after applying the summarize function because most people.

You can suppress this warning with a line near the top of the code

|# warning: false

or you can explicitly choose the first option by adding the argument

.drop="drop_last"

or the second option by adding the argument
...
ex_1 |>
  ...
  ggtitle("Graph drawn by Steve Simon on 2025-02-11") +
  xlab("Gender") +
  ylab("Average weight loss") -> plot_4
plot_4
```
...
tidy(m6) |>
  mutate(
    p.value =
      case_when(
        p.value >= 0.001 ~ glue("p = {round(p.value, 3)}"),
        p.value <  0.001 ~ "p < 0.001")) -> table_08
table_08
```
