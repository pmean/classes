...
The gt function produces a nice table, and allows many possible customizations. The two most important customization functions are format_number and tab_source_note. The format_number function allws you to round the results in your table, add extra text (like a percent sign), and insert commas for numbers 1,000 and larger. The tab_source_note function places a note at the bottom of the table. I use this to identify the author, date, and copyright restrictions. for me, it functions largely the same as the caption argument in the labs function in ggplot2.

For future assignments, please use gt for every table that you produce other than a listing of the data. Be sure to change the rounding of a column of numbers if needed, and put your own name, date, and copyright restrictions.
...
er_admits1 |>
  ...
  gt() |>
  fmt_number(pct, decimals=0, pattern="{x}%") |>
  tab_source_note("Steve Simon, 2026-02-11, CC0") -> moon_table
...
The pattern argument of the fmt_number function allows you to display a variable with extra text. In this example, the pattern function adds a percentage sign to the pct variable.
...
er_admits1 |>
  ...
  fmt_number(admission_sd, decimals=1) |>
  tab_source_note("Steve Simon, 2026-02-11, CC0") -> admits_table
...
er_admits1 |>
  ...
  tab_source_note("Steve Simon, 2026-02-11, CC0") -> admits_by_moon_table
...
If you leave the column name outof the fmt_number function of gt, then  
...
anova(lm_null_model, lm_moon_model) |> 
  ...
  tab_source_note("Steve Simon, 2026-02-11, CC0") -> anova_moon_table
...
lm_moon_model |>
  ...
  fmt_number(std.error, decimals=1) |>
  fmt_number(statistic, decimals=2) |>
  fmt_number(p.value, decimals=3) |>
  tab_source_note("Steve Simon, 2026-02-11, CC0") -> estimates_moon_table
