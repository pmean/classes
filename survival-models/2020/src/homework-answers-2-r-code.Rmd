...
# The last three lines in the yaml header above
# insure that the output is saved in the results
# folder. For details, see
# http://new.pmean.com/post/results-folder/
```
...
km_overall <- survfit(Surv(time_yrs, fstat==1)~1, data=whas500)
plot(km_overall, conf.int=FALSE)

table(whas500$gender, whas500$fstat)
km_by_gender <- 
  survfit(Surv(time_yrs, fstat==1)~gender, data=whas500)
plot(km_by_gender)

survdiff(Surv(time_yrs, fstat==1)~gender, data=whas500)
```
...
```{r cox-model-gender}
cox_gender <- coxph(
  Surv(time_yrs, fstat==1)~gender, data=whas500)
summary(cox_gender)
```
...
par(mar=c(2.1, 2.1, 0.6, 0.6))
km_by_age_group <- 
  survfit(Surv(time_yrs, fstat==1)~age_group, data=whas500)
plot(km_by_age_group)
survdiff(Surv(time_yrs, fstat==1)~age_group, data=whas500)

cox_age_group <- 
  coxph(Surv(time_yrs, fstat==1)~age_group, data=whas500)
print(cox_age_group)
...
```{r cox-model-age}
cox_age <- 
  coxph(Surv(time_yrs, fstat==1)~age, data=whas500)
summary(cox_age)
```
...
```{r save-everything}
save.image("../data/homework-answers-2-r-code.RData")
```